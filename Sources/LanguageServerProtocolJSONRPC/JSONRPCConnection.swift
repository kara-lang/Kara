...
public final class JSONRPCConnection {
  ...
  public func start(receiveHandler: MessageHandler, closeHandler: @escaping () -> () = {}) {
  ...
  func send(messageData: Data, handleCompletion: (() -> ())? = nil) {
...
extension JSONRPCConnection: Connection {
  ...
  public func send<Notification>(_ notification: Notification) where Notification: NotificationType {
    guard readyToSend() else { return }
    send { encoder in
      try encoder.encode(JSONRPCMessage.notification(notification))
    }
  }
  ...
  public func send<Request>(
    ...
    send { encoder in
      try encoder.encode(JSONRPCMessage.request(request, id: id))
    }
